...
public abstract class IndexedColumn<T> extends ColumnAbstract<T> {
	...
	this.docBitsetsMap = table.store
			.getMap("column." + columnId + ".idx.", ByteConverter.IntegerByteConverter.INSTANCE,
					ByteConverter.RoaringBitmapConverter);
	this.storedTermVectorMap = table.store
			.getMap("column." + columnId + ".tv", ByteConverter.IntegerByteConverter.INSTANCE,
    ...
    private void putTermVectorIdSet(Integer docId, Set<Integer> termIdSet) throws IOException {
    ...
    private void setTerms(Integer docId, Set<Integer> newTermIdSet) throws IOException {
	...
	boolean isIdentical = oldTermIdSet != null && oldTermIdSet.size() == newTermIdSet.size() && oldTermIdSet
			.containsAll(newTermIdSet);
	...
	// Check if the old and the new id set are identical
	boolean isIdentical = termIdSet != null && termIdSet.size() == 1 && termIdSet.contains(termId);
    ...
    @Override
    public void setValues(final Integer docId, Collection<Object> values) throws IOException {
    ...
    public CollectorInterface newFacetCollector(CollectorInterface collector, Map<Integer, LongCounter> termCounter) {
	rwl.r.lock();
	try {
    ...
    public void resolveFacetsIds(Map<Integer, LongCounter> termIdMap, Map<String, LongCounter> termMap)
		    throws IOException {
	if (termIdMap == null)
	    return;
	rwl.r.lock();
	try {
		...
		String term = storedInvertedDictionaryMap.get(entry.getKey()).toString();
